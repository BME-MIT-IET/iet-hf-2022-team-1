...
import { shaker_sort } from '../../src/shaker_sort/shaker_sort.js'

describe('shaker sort algorithm', () => {

    it('should return a sorted array given all elemenents are unique', () => {
        const array = [5, 8, 10, 2, 7, 1, 4, 3, 9, 6];

        const sorted = shaker_sort(array);
    ...
    it('should immediately break its loop if the input array is already sorted', () => {
        const array = [-5, -5, -4, -4, -3, -3, -2, -1, -1, -1, -1, 0, 0, 0, 1, 2, 2, 3, 4, 5];

        const sorted = shaker_sort(array);

        expect(sorted).to.be.eql([-5, -5, -4, -4, -3, -3, -2, -1, -1, -1, -1, 0, 0, 0, 1, 2, 2, 3, 4, 5]);
    });
});